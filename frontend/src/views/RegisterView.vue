  ...
  <div class="login-bg-new">
    <div class="login-card-new">
      ...
      <div class="login-title-block">
        <div class="login-title-new">Crie sua conta no</div>
      ...
      <form @submit.prevent="handleSubmit" class="login-form-new">
        ...
        <div class="register-link-new">
          <router-link to="/login">Já tem uma conta? <b>Faça login</b></router-link>
        </div>
      </form>
    </div>
  </div>
...
const v$ = useVuelidate(rules, { name, emailValue, password, confirmPassword })
...
const handleSubmit = async () => {
  ...
  try {
    ...
    toast.add({
      severity: 'error',
      summary: 'Erro',
      detail: error.response?.data?.error || 'Erro ao realizar registro',
      life: 3000
    })
  } finally {
    loading.value = false
